...
def create_eval_parser(base_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument('--batch_size', default=1024, type=int,
                        help='Batch size')
    parser.add_argument('--data_dir', default='./data_process', type=utils.check_is_dir,
                        help='Directory from which to load task data')
    ...
    parser.add_argument('--seq_info_file', default=None, type=str,
                        help='name of input seqs info file for simulated annealing')
    parser.add_argument('--mut_mode', default='both', type=str,
                        help='add mutations to chain heavy, light or both')
    parser.add_argument('--wt_anchor', action='store_true', help='always add mutations to the WT seq')
...
def run_antibody_engineer(args: typing.Optional[argparse.Namespace] = None) -> typing.Dict[str, float]:
    if args is None:
        base_parser = create_base_parser()
        parser = create_eval_parser(base_parser)
        args = parser.parse_args()
    ...
    arg_dict = vars(args)
    arg_names = inspect.getfullargspec(training.run_antibody_engineer).args
    ...
    return training.run_antibody_engineer(**eval_args)
