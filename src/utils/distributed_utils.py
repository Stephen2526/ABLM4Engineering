...
#import torch.multiprocessing as mp # not working for now 
import sys
import signal
...
def launch_process_group(func: typing.Callable,
    ...
    for local_rank in range(num_processes):
        ...
        current_env["CUDA_VISIBLE_DEVICES"] = int(local_rank)
    ...
    print('_>util/distributed_utils, sub-processes pids: {}'.format(process_context.pids()),flush=True)
    if not join:
